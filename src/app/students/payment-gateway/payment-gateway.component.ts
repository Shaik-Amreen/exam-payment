...
export class PaymentGatewayComponent implements OnInit {
 ...
 @Input() appdata : FormGroup;
  ...
  onPaymentDataAuthorized: google.payments.api.PaymentAuthorizedHandler = (paymentData : any) =>
  {
    ...
    {
          let a={date:this.appdata.value.date,
          ...
          this.http.post<any>('http://localhost:4000/payment/savesupplystudentdata',this.appdata.value).subscribe(
    ...
    {
      this.http.post<any>('http://localhost:4000/payment/savestudentdata',this.appdata.value).subscribe(
      res=> this.router.navigate(['/student/hallticket']),
      err=>console.log(err)
    )
    }
